...
Future<void> loadingFailed(Object? message,
  ...
  EasyLoading.dismiss();
  if (message is FirebaseException) {
    if (message.code == 'network-request-failed') {
      Navigator.push(
          context!, MaterialPageRoute(builder: (context) => BadNetwork()));
    }
  }
  if (message != null) {
    if (message is FirebaseException) {
      if (message.code == 'resource-exhausted') {
        showSnackBar(context!, "File too large");
      }
      if (message.code.startsWith('failed-precondition') ||
          message.code.startsWith('aborted') ||
          message.code.startsWith('cancelled')) {
